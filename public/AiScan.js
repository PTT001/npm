...
function showPrediction(predictions) {
  ...
  // console.log(predictionBuffer)

  // 只有當 buffer 滿時才進行 invoke
  if (predictionBuffer.length === bufferSize) {
    ...
    if (dominantPrediction) {
      EventManager.invoke({
        ...
        payload: {
          ...
          confidence: dominantPrediction.averageConfidence.toFixed(2)
        }
      })
    }
  }
...
function classifyFrame() {
  ...
  tf.tidy(() => {
    // 從視頻元素創建TensorFlow張量
    let tfImage = tf.browser.fromPixels(video)

    // 將圖像調整為模型需要的224x224尺寸-------------------------
    const resizedImage = tf.image.resizeBilinear(tfImage, [224, 224])

    //new
    const grayScaleOneChannel = tf.image.rgbToGrayscale(resizedImage)
    const grayScaleThreeChannels = tf.concat(
      [grayScaleOneChannel, grayScaleOneChannel, grayScaleOneChannel],
      2
    )
    ...
    //new
    const normalizedImage = grayScaleThreeChannels.div(tf.scalar(255))
